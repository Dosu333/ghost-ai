Add `Share` button to the editor navbar that opens the share dialog.
...
Use Clerk Backend API is enrich collaborator emails with:

- display name
- avatar image

If a Clerk user is not found for email, fall back to showing email only.
...
Enforce ownership server-sie for invite and remove actions
